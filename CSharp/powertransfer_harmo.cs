    ...
    class PowerTransferMod : IAssemblyPlugin
    {
        ...
        public static bool Updateprefix(float deltaTime, Camera cam, PowerTransfer __instance)
        {
            ...
            if (escapedTime > updateTimer)
            {
                ...
                {
                    var itemContainer = __instance.item.GetComponent<ItemContainer>();
                    if (itemContainer != null)
                    {
                        var fuse = itemContainer.Inventory.GetItemAt(0);
                        if (fuse == null || fuse.Condition <= 0)
                        {
                            //DebugConsole.NewMessage("Junction box has no fuse or fuse is broken. Decreasing condition.", Color.Red);
                            _.item.Condition -= 0.5f;
                            flagChangedConnections(_.item.connections);
                        }
                    }
                }
                escapedTime = 0;
